...
Accurate classification of sleep stages from electroencephalogram (EEG) signals is a fundamental component of the assessment of affective and physiological state, with direct relevance to sleep quality, emotional regulation, and neurological health. It plays an important role in long-term neural health monitoring and clinical sleep assessment. However, practical deployment of deep learning–based sleep staging systems remains challenging due to substantial inter-subject variability in EEG signals and the high computational cost of state-of-the-art (SOTA) models, particularly in wearable and clinical monitoring environments. In this work, a unified two-stage framework is proposed that jointly optimizes neural architecture design and model compression through the integration of differentiable architecture search (DARTS) and multi-level pruning strategies. In Stage~1, DARTS is employed to discover compact but expressive architectures under the joint influence of a bidirectional data pruning mechanism, which removes noisy, imbalanced, and ambiguous samples, and a differentiable operation pruning mechanism, which dynamically suppresses low-utility operations via temperature-controlled annealing. In Stage~2, the architecture obtained from Stage~1 is further refined by pruning post-search filters based on information capacity and independence, reducing the number of convolutional filters by 50\% while preserving classification accuracy. Experimental results on the Sleep-EDF-20 and Sleep-EDF-78 datasets demonstrate that the proposed method achieves robust and competitive SOTA performance while substantially reducing model complexity. Moreover, the resulting models enable real-time inference under resource-constrained conditions, supporting practical deployment in wearable and clinical EEG monitoring systems. These findings indicate that the proposed framework provides a scalable and resource-efficient solution for sleep-stage-based neural health monitoring across wearable, home-based, and clinical scenarios.
